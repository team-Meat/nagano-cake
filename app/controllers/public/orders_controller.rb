...
def confirm
  ...
  #   @order.street_address = session[:address]
  ...
  #   # saveができた段階でOrderモデルにorder_idが入る
    ...
    #   @item_order.item_id = cart.item.id
    #   @item_order.once_price = cart.item.price
  ...
  def shipping_params
   params.require(:order).permit(:postal_code, :street_address)
  end
end